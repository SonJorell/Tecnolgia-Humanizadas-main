...
<template>
  <div class="p-4 md:p-6 max-w-5xl mx-auto space-y-6 animate-fade-in">
    <div>
      ...
      <p class="text-sm text-text-muted mt-1">Revisa sugerencias, marca problemas resueltos y otorga recompensas.</p>
    </div>
    ...
    <div v-else class="space-y-4">
      <BaseCard v-for="fb in feedbacks" :key="fb.id" class="p-5 flex flex-col md:flex-row gap-4">
        <div class="flex items-center gap-3 md:w-1/4 flex-shrink-0">
          ...
          <div>
            <p class="font-bold text-text dark:text-dark-text text-sm">{{ fb.autor?.nombre || 'Usuario' }}</p>
            <p class="text-[10px] text-text-muted uppercase font-semibold">{{ fb.autor?.rol }}</p>
            <p class="text-[10px] text-text-light mt-0.5">{{ new Date(fb.creado_en).toLocaleDateString() }}</p>
          </div>
        </div>
        ...
        <div class="flex-1">
          ...
          <p class="text-sm text-text dark:text-dark-text">{{ fb.comentario || '(Sin comentario adicional)' }}</p>
        </div>

        <div class="md:w-1/4 flex flex-col justify-center gap-2 border-t md:border-t-0 md:border-l border-border dark:border-white/10 pt-4 md:pt-0 md:pl-4">
          <div v-if="fb.resuelto" class="flex flex-col items-center text-mint bg-mint-bg rounded-lg p-3">
            <CheckCircle :size="24" class="mb-1" />
            <span class="text-xs font-bold uppercase">Resuelto</span>
          </div>
          <template v-else>
            <div class="flex flex-col gap-1 mb-2">
              <label class="text-[10px] text-text-muted font-semibold uppercase">Recompensa (Útil)</label>
              <div class="flex items-center gap-2 text-xs font-bold text-text dark:text-dark-text">
                🪙 <input v-model.number="monedasPremio" type="number" class="w-16 h-6 px-1 rounded bg-surface dark:bg-dark-card border border-border dark:border-white/10" />
                ⭐ <input v-model.number="xpPremio" type="number" class="w-16 h-6 px-1 rounded bg-surface dark:bg-dark-card border border-border dark:border-white/10" />
              </div>
            </div>
            <div class="grid grid-cols-2 gap-2">
              ...
              <BaseButton variant="secondary" size="sm" :disabled="processing" class="w-full justify-center px-0 text-xs text-text-muted" @click="resolverFeedback(fb.id, false)">
                Cerrar
              </BaseButton>
            </div>
          </template>
      ...
      <div v-if="feedbacks.length === 0" class="text-center py-10 bg-surface dark:bg-dark-card2 rounded-xl">
        <p class="text-text-muted">No hay feedback registrado.</p>
      </div>
    </div>
  </div>
</template>
