  ...
  <div class="p-4 md:p-6 space-y-6 animate-fade-in max-w-5xl mx-auto">
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="text-sm text-text-muted mt-1">Supervisa roles y estado del sistema de los usuarios.</p>
      </div>
      
      <div class="relative w-full md:w-64 mt-4 md:mt-0">
        <Search :size="16" class="absolute left-3 top-1/2 -translate-y-1/2 text-text-muted" />
        <input v-model="search" type="text" placeholder="Buscar usuario..." class="w-full h-10 pl-9 pr-4 bg-surface dark:bg-dark-card border border-border dark:border-white/10 rounded-lg text-sm outline-none focus:ring-2 focus:ring-violet/50" />
      </div>
    </div>
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-left border-collapse">
          ...
          <tbody class="divide-y divide-border dark:divide-white/10 text-sm">
            <tr v-if="loading">
              <td colspan="4" class="p-4 text-center text-text-muted">Cargando...</td>
            </tr>
            <tr v-else-if="getFilteredUsers().length === 0">
              <td colspan="4" class="p-4 text-center text-text-muted">No se encontraron usuarios.</td>
            </tr>
            <tr v-else v-for="user in getFilteredUsers()" :key="user.id" class="hover:bg-surface/50 dark:hover:bg-dark-card2/50 transition-colors">
              <td class="px-4 py-3 font-medium text-text dark:text-dark-text flex items-center gap-2">
              ...
              <td class="px-4 py-3 text-center">
                <span class="inline-flex items-center gap-1 text-mint text-xs font-bold">
