...
const estadoConexionLabel = computed(() => {
 ...
 case 'ONLINE_SUPABASE': return { label: 'Internet + Supabase', color: 'text-secondary', bg: 'bg-secondary-container ', icon: Wifi }
 case 'ONLINE_LAN_ONLY': return { label: 'Solo Red Local', color: 'text-tertiary', bg: 'bg-tertiary-container ', icon: Wifi }
 case 'OFFLINE_TOTAL': return { label: 'Sin Conexión', color: 'text-error', bg: 'bg-error-container ', icon: WifiOff }
 default: return { label: 'Desconocido', color: 'text-on-surface-variant', bg: 'bg-surface-container-low', icon: WifiOff }
 }
})
...
<template>
 ...
 <p class="text-sm text-on-surface-variant mt-1">Estado del sistema offline-first, sincronización y almacenamiento local.</p>
 </div>
 ...
 appStore.conexion === 'ONLINE_SUPABASE' ? 'border-secondary/30' : 
 appStore.conexion === 'ONLINE_LAN_ONLY' ? 'border-tertiary/30' : 'border-error/30']">
 ...
 <p class="text-xs text-on-surface-variant">Los datos locales siempre están disponibles, independiente de la red.</p>
 </div>
 </div>
 <BaseBadge
 ...
 <h3 class="font-headline-md font-bold text-base text-on-surface mb-4 flex items-center gap-2">
 ...
 <span class="text-on-surface-variant">IndexedDB</span>
 <span class="font-bold" :class="idbPorcentaje > 80 ? 'text-error' : idbPorcentaje > 60 ? 'text-tertiary' : 'text-primary'">
 ...
 :class="idbPorcentaje > 80 ? 'bg-error' : idbPorcentaje > 60 ? 'bg-tertiary' : 'bg-primary'"
 ...
 <span class="text-xs text-on-surface">Base de datos local</span>
 </div>
 <span class="text-xs font-bold text-on-surface">{{ (almacenamiento.idbUsado - almacenamiento.cacheArchivos).toFixed(1) }} MB</span>
 </div>
 <div class="flex items-center justify-between p-3 bg-surface-container-low rounded-xl">
 <div class="flex items-center gap-2">
 <div class="w-3 h-3 rounded-full bg-tertiary"></div>
 <span class="text-xs text-on-surface">Caché de archivos</span>
 </div>
 <span class="text-xs font-bold text-on-surface">{{ almacenamiento.cacheArchivos }} MB</span>
 </div>
 </div>
 ...
 <div class="space-y-4">
 <!-- Estado -->
 <div class="flex items-center justify-between p-3 rounded-xl" :class="servidorLocal.estado === 'activo' ? 'bg-secondary-container ' : 'bg-error-container '">
 <div class="flex items-center gap-2">
 <div class="w-2.5 h-2.5 rounded-full" :class="servidorLocal.estado === 'activo' ? 'bg-secondary animate-pulse' : 'bg-error'"></div>
 <span class="text-sm font-semibold" :class="servidorLocal.estado === 'activo' ? 'text-secondary' : 'text-error'">
 {{ servidorLocal.estado === 'activo' ? 'Activo' : 'Inactivo' }}
 </span>
 </div>
 <span class="text-xs text-on-surface-variant font-mono">{{ servidorLocal.ip }}:{{ servidorLocal.puerto }}</span>
 </div>

 <!-- Metrics -->
 <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
 <div class="bg-surface-container-low rounded-xl p-3 text-center">
 <p class="text-xs text-on-surface-variant mb-1">Latencia</p>
 <p class="font-headline-md font-extrabold text-xl" :class="servidorLocal.latencia < 100 ? 'text-secondary' : servidorLocal.latencia < 300 ? 'text-tertiary' : 'text-error'">
 {{ servidorLocal.latencia }}ms
 </p>
 </div>
 <div class="bg-surface-container-low rounded-xl p-3 text-center">
 <p class="text-xs text-on-surface-variant mb-1">Clientes</p>
 <p class="font-headline-md font-extrabold text-xl text-primary">{{ servidorLocal.clientesConectados }}</p>
 </div>
 </div>
 ...
 <div>
 <div class="flex justify-between text-xs mb-1.5">
 <span class="text-on-surface-variant">Velocidad de sync promedio</span>
 <span class="font-bold text-on-surface">{{ syncStatus.promedioMs }}ms</span>
 </div>
 ...
 <p class="text-[11px] text-on-surface-variant pt-2 border-t border-border-subtle">
 ...
 <BaseCard class="lg:col-span-1">
 <h3 class="font-headline-md font-bold text-base text-on-surface mb-4 flex items-center gap-2">
 <RefreshCw :size="18" class="text-tertiary" /> Cola de Sincronización
 </h3>
 ...
 <p class="text-[10px] text-on-surface-variant uppercase font-semibold mb-1">Local</p>
 <div class="w-8 h-8 mx-auto rounded-full bg-primary-bg flex items-center justify-center">
 ...
 <div class="flex-1 text-center">
 <p class="text-[10px] text-on-surface-variant uppercase font-semibold mb-1">Nube</p>
 <div class="w-8 h-8 mx-auto rounded-full bg-secondary-container flex items-center justify-center">
 <Shield :size="14" class="text-secondary" />
 </div>
 </div>
 </div>
 ...
 <div class="flex items-center gap-2">
 <Clock :size="14" class="text-tertiary" />
 <span class="text-xs text-on-surface">Pendientes</span>
 </div>
 <span class="text-xs font-bold text-tertiary">{{ syncStatus.pendientes }}</span>
 </div>
 <div class="flex items-center justify-between">
 <div class="flex items-center gap-2">
 <XCircle :size="14" class="text-error" />
 <span class="text-xs text-on-surface">Fallidos</span>
 </div>
 <span class="text-xs font-bold text-error">{{ syncStatus.fallidos }}</span>
 </div>
 <div class="flex items-center justify-between">
 <div class="flex items-center gap-2">
 <CheckCircle2 :size="14" class="text-secondary" />
 <span class="text-xs text-on-surface">Exitosos</span>
 </div>
 <span class="text-xs font-bold text-secondary">{{ syncStatus.exitosos }}</span>
 </div>
 </div>
...
<table class="w-full text-left border-collapse">
 ...
 <span class="font-medium text-on-surface text-sm">{{ d.nombre }}</span>
 </div>
 </td>
 <td class="px-4 py-3">
 ...
 <div class="w-2 h-2 rounded-full" :class="d.estado === 'online' ? 'bg-secondary animate-pulse' : 'bg-text-light'"></div>
 <span class="text-xs font-semibold" :class="d.estado === 'online' ? 'text-secondary' : 'text-on-surface-variant'">
 {{ d.estado === 'online' ? 'Online' : 'Offline' }}
 </span>
 </div>
 </td>
 <td class="px-4 py-3 text-center text-xs text-on-surface-variant">{{ d.ultimaConexion }}</td>
 <td class="px-4 py-3 text-center text-xs font-bold text-on-surface">{{ d.almacenamientoLocal }}</td>
 </tr>
 </tbody>
 </table>
</div>
 ...
 <!-- Logs de Sistema -->
 <div class="space-y-4">
 <h2 class="font-headline-md font-bold text-lg text-on-surface">📋 Registro del Sistema</h2>
 <BaseCard class="p-0 divide-y divide-border">
 <div
 v-for="log in logsRecientes" :key="log.id"
 class="flex items-start gap-3 p-4 hover:bg-surface-container-low/50 transition-colors"
 >
 <div class="mt-0.5 flex-shrink-0">
 <CheckCircle2 v-if="log.tipo === 'success'" :size="16" class="text-secondary" />
 <AlertTriangle v-else-if="log.tipo === 'warning'" :size="16" class="text-tertiary" />
 <XCircle v-else-if="log.tipo === 'error'" :size="16" class="text-error" />
 <Activity v-else :size="16" class="text-primary" />
 </div>
 <div class="flex-1 min-w-0">
 <p class="text-sm text-on-surface leading-snug">{{ log.mensaje }}</p>
 <p class="text-[11px] text-on-surface-variant mt-0.5">{{ log.tiempo }}</p>
 </div>
 </div>
 </BaseCard>
 </div>
