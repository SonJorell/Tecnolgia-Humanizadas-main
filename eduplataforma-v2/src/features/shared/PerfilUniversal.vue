...
<template>
  <div class="p-4 md:p-6 max-w-4xl mx-auto space-y-6 animate-fade-in pb-20">
    <div class="flex items-center gap-3">
      ...
      <div>
        <h1 class="font-display font-extrabold text-2xl md:text-3xl text-text dark:text-dark-text">Mi Perfil</h1>
        <p class="text-sm text-text-muted mt-1">Personaliza tu cuenta y gestiona tu seguridad.</p>
      </div>
    </div>

    <!-- Mensajes de Feedback -->
    <div v-if="successMsg || errorMsg" class="flex items-start gap-3 p-4 rounded-xl border" :class="errorMsg ? 'bg-danger-bg/50 border-danger/30' : 'bg-mint-bg/50 border-mint/30'">
      <AlertTriangle v-if="errorMsg" class="text-danger flex-shrink-0" />
      <CheckCircle v-else class="text-mint flex-shrink-0" />
      <div class="flex-1">
        <p class="font-medium text-sm" :class="errorMsg ? 'text-danger' : 'text-mint'">
          {{ errorMsg || successMsg }}
        </p>
      </div>
      <button @click="dismissMsg" class="text-text-muted hover:text-text transition-colors">
        <X :size="18" />
      </button>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
      ...
      <div class="md:col-span-2 space-y-6">
        ...
        <BaseCard>
          <h3 class="font-bold text-lg mb-4 text-text dark:text-dark-text border-b border-border dark:border-white/10 pb-2">Información Personal</h3>
          
          <div class="relative w-full rounded-t-xl h-32 bg-primary/20 mb-12 flex justify-center bg-cover bg-center" :style="{ backgroundImage: authStore.perfil?.banner_activo_id ? `url(${authStore.perfil.banner_activo_id})` : '' }">
            <!-- Avatar Previo -->
            <div class="absolute -bottom-10 flex flex-col items-center gap-3">
              <div class="w-24 h-24 rounded-full bg-surface dark:bg-dark-surface border-4 border-surface dark:border-dark-card overflow-hidden flex items-center justify-center flex-shrink-0 shadow-lg relative" :class="{ 'ring-4 ring-amber-400': authStore.perfil?.marco_activo_id === 'gold', 'ring-4 ring-cyan-400': authStore.perfil?.marco_activo_id === 'diamond' }">
          ...
          <div class="flex flex-col sm:flex-row gap-6 mb-6 px-6">
            <div class="flex flex-col items-center gap-2 justify-center w-full sm:w-1/3">
              <input type="file" ref="fileInput" accept="image/*" class="hidden" @change="handleFileUpload" />
              <button @click="triggerFileUpload" class="text-xs font-semibold text-primary bg-primary/10 hover:bg-primary/20 px-3 py-1.5 rounded-full transition-colors flex items-center gap-1.5">
            ...
            <form @submit.prevent="savePerfil" class="flex-1 space-y-4">
              <div class="space-y-1">
                ...
                <input v-model="perfilForm.nombre" required type="text" class="w-full px-3 py-2 bg-surface dark:bg-dark-card border border-border dark:border-white/10 rounded-lg text-sm" />
              </div>
              <div class="space-y-1">
                <label class="text-xs font-semibold text-text-muted">Teléfono</label>
                <input v-model="perfilForm.telefono" type="text" placeholder="+569..." class="w-full px-3 py-2 bg-surface dark:bg-dark-card border border-border dark:border-white/10 rounded-lg text-sm" />
              </div>
              <div class="space-y-1">
                <label class="text-xs font-semibold text-text-muted">Correo (Solo lectura)</label>
                <input :value="authStore.perfil?.email" disabled type="email" class="w-full px-3 py-2 bg-surface/50 dark:bg-dark-surface border border-border dark:border-white/5 rounded-lg text-sm text-text-muted cursor-not-allowed" />
              </div>
        ...
        <BaseCard>
          <h3 class="font-bold text-sm mb-4 text-text dark:text-dark-text flex items-center gap-2">
            <ImageIcon :size="16" class="text-text-muted" /> Avatares Predefinidos
          </h3>
          <div class="flex flex-wrap gap-4">
            <button 
              ...
              :class="perfilForm.avatar_url === url ? 'border-primary' : 'border-transparent bg-surface dark:bg-dark-surface'"
            >
        ...
        <BaseCard v-if="gamificacionStore.inventarioPremios.length > 0" class="animate-slide-up">
          ...
          <p class="text-xs text-text-muted mb-4">Haz clic en un ítem para equiparlo en tu perfil.</p>
          <div class="flex flex-wrap gap-4">
            <button 
              v-for="(item, idx) in gamificacionStore.inventarioPremios" 
              :key="idx"
              @click="equiparItem(item)"
              class="relative rounded-lg overflow-hidden border-2 border-transparent hover:border-primary transition-all p-1 bg-surface dark:bg-dark-surface"
            >
      ...
      <div class="space-y-6">
        <BaseCard class="border-l-4 border-l-amber">
          <h3 class="font-bold text-lg mb-4 text-text dark:text-dark-text border-b border-border dark:border-white/10 pb-2 flex items-center gap-2">
            <Key :size="18" class="text-amber" /> Seguridad
          </h3>
          <form @submit.prevent="savePassword" class="space-y-4">
            <div class="space-y-1">
              <label class="text-xs font-semibold text-text-muted">Nueva Contraseña</label>
              <input v-model="pwForm.nueva" required type="password" class="w-full px-3 py-2 bg-surface dark:bg-dark-card border border-border dark:border-white/10 rounded-lg text-sm" placeholder="Mínimo 6 caracteres" />
            </div>
            <div class="space-y-1">
              <label class="text-xs font-semibold text-text-muted">Confirmar Contraseña</label>
              <input v-model="pwForm.confirmar" required type="password" class="w-full px-3 py-2 bg-surface dark:bg-dark-card border border-border dark:border-white/10 rounded-lg text-sm" placeholder="Repite la contraseña" />
            </div>
