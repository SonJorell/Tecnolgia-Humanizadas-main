...
<template>
 ...
 <p class="text-sm text-on-surface-variant mt-1">Administra la información, banner y objetivos de tus clases.</p>
 </div>
 ...
 <div>
 <h3 class="font-bold text-lg text-on-surface leading-tight">{{ curso.nombre }}</h3>
 <p class="text-xs font-semibold text-primary mt-1">{{ curso.nivel }}</p>
 </div>
 <span class="text-3xl">{{ curso.icono }}</span>
 </div>
 
 <p class="text-sm text-on-surface-variant mb-4 flex-1 line-clamp-3">
 {{ curso.descripcion || 'Sin descripción. Haz clic en editar para agregar detalles sobre este curso.' }}
 </p>

 <div class="flex items-center gap-4 text-sm font-semibold text-on-surface mb-4 border-t border-border pt-4">
 ...
 <BaseButton variant="ghost" class="w-full justify-center text-on-surface-variant" @click="startEdit(curso)">
 ...
 <textarea id="editForm_descripcion" name="editForm_descripcion" v-model="editForm.descripcion" class="w-full p-2 bg-surface-container-low border border-border rounded-lg text-sm outline-none" rows="3"></textarea>
 </div>
 <div>
 <label class="block text-xs font-bold text-on-surface-variant uppercase mb-1">URL de Portada (Banner)</label>
 <input id="editForm_banner_url" name="editForm_banner_url" v-model="editForm.banner_url" type="text" placeholder="https://..." class="w-full p-2 bg-surface-container-low border border-border rounded-lg text-sm outline-none" />
 </div>
 <div>
 <label class="block text-xs font-bold text-on-surface-variant uppercase mb-1">Objetivos (uno por línea)</label>
 <textarea id="editForm_objetivosText" name="editForm_objetivosText" v-model="editForm.objetivosText" class="w-full p-2 bg-surface-container-low border border-border rounded-lg text-sm outline-none" rows="3" placeholder="Aprender matemáticas..."></textarea>
 </div>
 </div>
 
 <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 mt-4 pt-4 border-t border-border">
 ...
 <div v-if="docenteStore.cursos.length === 0" class="col-span-full text-center py-12 text-on-surface-variant">
