...
<template>
  <div class="p-4 md:p-6 max-w-6xl mx-auto space-y-6 animate-fade-in">
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-4">
      <div>
        <h1 class="font-display font-extrabold text-2xl md:text-3xl text-text dark:text-dark-text">🧑‍🎓 Tus Alumnos</h1>
        <p class="text-sm text-text-muted mt-1">Revisa el rendimiento y otorga recompensas manuales por participación.</p>
      </div>
    </div>
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
      <div class="flex flex-col md:flex-row gap-3 w-full md:w-auto">
        <div class="relative shadow-sm w-full md:w-64">
          ...
          <input v-model="search" type="text" placeholder="Buscar alumno..." class="w-full h-10 pl-9 pr-4 bg-card dark:bg-dark-card border border-border dark:border-white/10 rounded-lg text-sm outline-none focus:ring-2 focus:ring-primary/50" />
        </div>
        
        <div class="relative shadow-sm w-full md:w-48">
          <Filter :size="16" class="absolute left-3 top-1/2 -translate-y-1/2 text-text-muted" />
          <select v-model="filterCurso" class="w-full h-10 pl-9 pr-4 bg-card dark:bg-dark-card border border-border dark:border-white/10 rounded-lg text-sm outline-none appearance-none focus:ring-2 focus:ring-primary/50">
    ...
    <BaseCard v-else class="overflow-hidden">
      <div class="overflow-x-auto">
        <table class="w-full text-left border-collapse">
          <thead>
            <tr class="bg-surface dark:bg-dark-card2 border-b border-border dark:border-white/10 text-xs uppercase font-bold text-text-muted">
              <th class="p-4 w-12">
                <input type="checkbox" :checked="allSelected" @change="toggleSelectAll" class="rounded text-primary focus:ring-primary w-4 h-4 cursor-pointer" />
              </th>
          ...
          <tbody class="divide-y divide-border dark:divide-white/10">
            <tr v-for="alumno in alumnosFiltrados" :key="alumno.id" class="hover:bg-surface dark:hover:bg-dark-card2 transition-colors group">
              <td class="p-4">
                <input type="checkbox" :checked="selectedAlumnos.has(alumno.id)" @change="toggleSelect(alumno.id)" class="rounded text-primary focus:ring-primary w-4 h-4 cursor-pointer" />
              </td>
              <td class="p-4">
                <div class="flex items-center gap-3">
                  <div class="w-8 h-8 rounded-full bg-primary-bg dark:bg-primary/20 text-primary flex items-center justify-center font-bold text-xs flex-shrink-0 overflow-hidden">
                  ...
                  <div>
                    <p class="font-bold text-text dark:text-dark-text text-sm cursor-pointer hover:text-primary transition-colors" @click="router.push('/docente/alumnos/' + alumno.id)">{{ alumno.nombre }}</p>
                  </div>
                </div>
              </td>
              <td class="p-4 text-sm text-text-muted">{{ alumno.curso_nombre || 'Sin curso' }}</td>
              <td class="p-4 text-center">
                <BaseBadge variant="primary" size="sm">Nvl {{ alumno.nivel }}</BaseBadge>
              </td>
              <td class="p-4 text-center">
                <div class="w-full bg-border dark:bg-white/10 rounded-full h-2 max-w-[100px] mx-auto">
                  <div class="bg-amber-500 h-2 rounded-full" :style="{ width: `${Math.min(100, (alumno.xp || 0) / 10)}%` }"></div>
                </div>
                <p class="text-[10px] text-amber-600 dark:text-amber-400 font-bold mt-1">{{ alumno.xp || 0 }} XP</p>
              </td>
              <td class="p-4 text-right opacity-0 group-hover:opacity-100 transition-opacity">
                <div class="flex items-center justify-end gap-2">
                  <BaseButton variant="ghost" size="sm" class="px-2" @click="irAMensaje(alumno.id)" title="Enviar Mensaje">
                    <Mail :size="16" class="text-text-muted hover:text-primary" />
                  </BaseButton>
            ...
            <tr v-if="alumnosFiltrados.length === 0">
              <td colspan="6" class="p-8 text-center text-text-muted">No se encontraron alumnos.</td>
            </tr>
          </tbody>
        </table>
      </div>
