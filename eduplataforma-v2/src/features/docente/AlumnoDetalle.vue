...
<template>
  <div class="p-4 md:p-6 max-w-4xl mx-auto space-y-6 animate-fade-in">
    <button @click="router.back()" class="flex items-center gap-2 text-sm text-text-muted hover:text-primary transition-colors mb-4">
    ...
    <template v-else-if="alumno">
      ...
      <BaseCard class="flex flex-col md:flex-row items-center gap-6 p-6">
        ...
        <div class="text-center md:text-left flex-1">
          <h1 class="text-2xl font-extrabold font-display text-text dark:text-dark-text">{{ alumno.nombre || 'Usuario Desconocido' }}</h1>
          <p class="text-text-muted mt-1 text-sm">{{ alumno.curso?.nombre || 'Sin curso asignado' }}</p>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <BaseCard class="p-5">
          <h3 class="font-bold mb-4 flex items-center gap-2 text-text dark:text-dark-text"><CheckCircle :size="18" class="text-mint"/> Últimas Entregas</h3>
          <div v-if="entregas.length === 0" class="text-sm text-text-muted">No hay entregas recientes.</div>
          <ul class="space-y-3">
            <li v-for="ent in entregas" :key="ent.id" class="flex justify-between items-center text-sm p-2 rounded-md hover:bg-surface dark:hover:bg-dark-card2">
              <span class="truncate pr-2">{{ ent.material?.titulo || 'Tarea eliminada' }}</span>
              <span :class="['px-2 py-0.5 rounded text-[10px] font-bold uppercase', 
                ent.estado === 'revisado' ? 'bg-mint-bg text-mint' : 'bg-amber-100 text-amber-700'
              ]">{{ ent.estado }}</span>
            </li>
          </ul>
        </BaseCard>
        ...
        <BaseCard class="p-5">
          <h3 class="font-bold mb-4 flex items-center gap-2 text-text dark:text-dark-text"><Trophy :size="18" class="text-primary"/> Acciones Rápidas</h3>
          <div class="space-y-3">
      ...
      <!-- Modal Asignar Logro -->
      <div v-if="showAssignModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-text/50 dark:bg-dark-bg/80 backdrop-blur-sm animate-fade-in">
        <div class="bg-card dark:bg-dark-card rounded-2xl w-full max-w-md shadow-modal overflow-hidden flex flex-col">
          <div class="p-4 border-b border-border dark:border-white/10 bg-surface dark:bg-dark-card2 flex justify-between items-center">
            <h2 class="font-bold text-text dark:text-dark-text">Otorgar Premio Especial</h2>
          </div>
          <div class="p-4 max-h-96 overflow-y-auto space-y-3">
            <div v-for="logro in docenteStore.logrosDocente" :key="logro.id" class="p-3 border border-border dark:border-white/10 rounded-xl flex items-center justify-between hover:bg-surface dark:hover:bg-dark-card2 cursor-pointer transition-colors" @click="asignarLogro(logro)">
              <div class="flex items-center gap-3">
                <span class="text-3xl">{{ logro.icono }}</span>
                <div>
                  <p class="font-bold text-text dark:text-dark-text text-sm">{{ logro.nombre }}</p>
                  <div class="flex gap-2 mt-1">
                    <span class="text-[10px] bg-amber-500/10 text-amber-500 px-1.5 py-0.5 rounded font-bold">+{{ logro.recompensa_xp }} XP</span>
                  </div>
                </div>
              </div>
              <BaseButton variant="ghost" size="sm">Dar</BaseButton>
            </div>
            <div v-if="docenteStore.logrosDocente.length === 0" class="text-center py-6 text-text-muted text-sm flex flex-col items-center">
              <AlertTriangle class="text-amber-500 mb-2" :size="24"/>
          ...
          <div class="p-4 border-t border-border dark:border-white/10 flex justify-end">
