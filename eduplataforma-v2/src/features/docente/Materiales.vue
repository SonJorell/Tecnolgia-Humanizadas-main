...
<template>
 ...
 <div>
 <h1 class="font-headline-md font-extrabold text-2xl md:text-3xl text-on-surface">📚 Materiales</h1>
 <p class="text-sm text-on-surface-variant mt-1">Sube archivos, guías y crea tareas para tus alumnos.</p>
 </div>
 ...
 <!-- Lista -->
 <div class="bg-surface-container-lowest border border-border rounded-2xl shadow-sm overflow-hidden">
 <div class="p-4 border-b border-border bg-surface-container-low flex flex-col sm:flex-row items-center gap-3">
 ...
 class="w-full h-9 pl-9 pr-4 bg-surface-container-lowest border border-border rounded-lg text-sm outline-none focus:ring-2 focus:ring-primary/50"
 />
 </div>
 <select id="filterCurso" name="filterCurso" v-model="filterCurso" class="w-full sm:w-48 h-9 px-3 bg-surface-container-lowest border border-border rounded-lg text-sm outline-none">
 ...
 <div v-if="docenteStore.loading" class="p-8 text-center text-on-surface-variant">Cargando materiales...</div>
 <div v-else-if="materialesList.length === 0" class="p-8 text-center text-on-surface-variant">No has subido ningún material aún.</div>
 
 <div v-else v-for="mat in materialesList" :key="mat.id" class="p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:bg-surface-container-low transition-colors">
 <div class="flex items-start sm:items-center gap-4 min-w-0">
 <div class="w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0" :class="mat.tipo === 'video' ? 'bg-tertiary-container text-tertiary' : mat.tipo === 'enlace' ? 'bg-violet-bg text-violet' : 'bg-primary-bg text-primary'">
 <FileVideo v-if="mat.tipo === 'video'" :size="20" />
 <FilePlus v-else-if="mat.tipo === 'enlace'" :size="20" />
 <FileText v-else :size="20" />
 </div>
 <div>
 <p class="font-semibold text-on-surface text-sm sm:text-base">{{ mat.titulo }}</p>
 <div class="flex flex-wrap items-center gap-2 text-xs mt-1">
 <span class="text-on-surface-variant font-medium">{{ docenteStore.cursos.find(c => c.id === mat.curso_id)?.nombre || 'Curso' }}</span>
 <span class="text-on-surface-light">&bull;</span>
 <span class="text-on-surface-variant">{{ new Date(mat.creado_en).toLocaleDateString() }}</span>
 <span v-if="mat.es_tarea" class="text-secondary font-bold uppercase text-[10px] px-1.5 py-0.5 rounded bg-secondary-container">Tarea</span>
 </div>
 </div>
 </div>
 ...
 <BaseButton variant="ghost" size="sm" class="text-error hover:bg-error-container" @click="eliminarMaterial(mat.id)">Eliminar</BaseButton>
 </div>
 </div>
 </div>
 </div>
 ...
 <!-- Modal Subida -->
 <div v-if="showUploadModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-text/50 backdrop-blur-sm animate-fade-in">
 <div class="bg-surface-container-lowest rounded-2xl w-full max-w-lg shadow-modal overflow-hidden flex flex-col max-h-[90vh]">
 <div class="p-4 border-b border-border bg-surface-container-low flex justify-between items-center">
 <h2 class="font-bold text-on-surface">{{ isEditing ? 'Editar Material' : 'Subir Nuevo Material' }}</h2>
 </div>
 
 <div class="p-6 overflow-y-auto space-y-4">
 <div>
 <label class="block text-sm font-semibold mb-1">Título</label>
 <input id="newMaterial_titulo" name="newMaterial_titulo" v-model="newMaterial.titulo" type="text" class="w-full bg-surface-container-low border border-border rounded-xl px-4 py-2.5 outline-none focus:ring-2 focus:ring-primary/50" />
 </div>
 
 <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
 <div>
 <label class="block text-sm font-semibold mb-1">Curso</label>
 <select id="newMaterial_curso_id" name="newMaterial_curso_id" v-model="newMaterial.curso_id" class="w-full bg-surface-container-low border border-border rounded-xl px-4 py-2.5 outline-none">
 ...
 <select id="newMaterial_tipo" name="newMaterial_tipo" v-model="newMaterial.tipo" class="w-full bg-surface-container-low border border-border rounded-xl px-4 py-2.5 outline-none">
 ...
 <input id="newMaterial_archivo_url" name="newMaterial_archivo_url" v-model="newMaterial.archivo_url" type="url" placeholder="https://..." class="w-full bg-surface-container-low border border-border rounded-xl px-4 py-2.5 outline-none focus:ring-2 focus:ring-primary/50" />
 <p class="text-xs text-on-surface-variant mt-1">Opcional. Pega aquí el enlace de Google Drive, YouTube u otra plataforma pública.</p>
 </div>
 ...
 <p class="text-sm text-on-surface-variant"><span class="font-bold text-primary">Haz clic para subir</span> o arrastra y suelta</p>
 </div>
 <input id="input_file_53" name="input_file_53" type="file" class="hidden" multiple @change="handleFileUpload" />
 </label>

 <!-- Lista de Archivos -->
 <div v-if="newMaterial.archivos.length > 0" class="mt-3 space-y-2">
 <div v-for="(file, idx) in newMaterial.archivos" :key="idx" class="flex items-center justify-between p-2 bg-surface-container-low rounded-lg border border-border">
 <div class="flex items-center gap-2 overflow-hidden">
 <FileText :size="16" class="text-primary flex-shrink-0" />
 <span class="text-xs truncate font-medium text-on-surface">{{ file.nombre }}</span>
 </div>
 <button type="button" @click="removeArchivo(idx)" class="text-error hover:text-error/80 px-2 font-bold">&times;</button>
 </div>
 </div>
 </div>

 <label class="flex items-center gap-2 p-3 bg-primary-bg border border-primary/20 rounded-xl cursor-pointer hover:bg-primary/20 transition-colors">
 <input id="newMaterial_es_tarea" name="newMaterial_es_tarea" v-model="newMaterial.es_tarea" type="checkbox" class="w-4 h-4 text-primary rounded focus:ring-primary" />
 <div>
 <span class="font-bold text-sm text-primary">Activar Buzón de Entrega (Es Tarea)</span>
 <p class="text-xs text-primary/70">Permite que el alumno suba su archivo resuelto y reciba calificación/XP.</p>
 </div>
 </label>

 <div v-if="newMaterial.es_tarea" class="grid grid-cols-1 sm:grid-cols-2 gap-4 animate-fade-in">
 <div>
 <label class="block text-sm font-semibold mb-1">XP al Entregar</label>
 <input id="newMaterial_xp_base" name="newMaterial_xp_base" v-model="newMaterial.xp_base" type="number" step="10" class="w-full bg-surface-container-low border border-border rounded-xl px-4 py-2.5 outline-none" />
 </div>
 <div>
 <label class="block text-sm font-semibold mb-1">Vencimiento</label>
 <input id="newMaterial_fecha_vencimiento" name="newMaterial_fecha_vencimiento" v-model="newMaterial.fecha_vencimiento" type="datetime-local" class="w-full bg-surface-container-low border border-border rounded-xl px-4 py-2.5 outline-none" />
 </div>
 </div>
