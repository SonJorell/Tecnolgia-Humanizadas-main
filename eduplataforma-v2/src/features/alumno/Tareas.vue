  ...
  <div class="p-4 md:p-6 max-w-4xl mx-auto space-y-6 animate-fade-in">
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-4">
      <div>
        ...
        <p class="text-sm text-text-muted mt-1">Organiza tu tiempo y gana experiencia.</p>
      </div>
      
      <!-- Filtros -->
      <div class="flex bg-surface dark:bg-dark-card2 p-1 rounded-xl w-full md:w-auto">
        <button
          :class="['flex-1 md:w-32 py-1.5 text-sm font-medium rounded-lg transition-all', filtro === 'pendientes' ? 'bg-card dark:bg-dark-card shadow-sm text-text dark:text-dark-text' : 'text-text-muted']"
          @click="filtro = 'pendientes'"
        >
          Pendientes ({{ tareasStore.pendientes.length }})
        </button>
        <button
          :class="['flex-1 md:w-32 py-1.5 text-sm font-medium rounded-lg transition-all', filtro === 'completadas' ? 'bg-card dark:bg-dark-card shadow-sm text-text dark:text-dark-text' : 'text-text-muted']"
    ...
    <div class="space-y-3">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center gap-4 justify-between">
          ...
          <div class="flex items-start gap-3">
            <div 
              ...
              :class="filtro === 'pendientes' ? 'bg-amber-bg text-amber dark:bg-amber/10' : 'bg-mint-bg text-mint dark:bg-mint/10'"
            >
            ...
            <div>
              ...
              <p class="font-semibold text-text dark:text-dark-text text-base leading-tight mb-1.5">
              ...
              <div class="flex flex-wrap items-center gap-2">
                ...
                <span class="text-xs font-bold text-primary bg-primary-bg dark:bg-primary/10 px-2 py-0.5 rounded-full">
                  +{{ tarea.xp }} XP
                </span>
              </div>
            </div>
      ...
      <div v-if="listaTareas.length === 0" class="text-center py-16 bg-card dark:bg-dark-card shadow-sm rounded-2xl">
        <p class="text-5xl mb-4">
          {{ filtro === 'pendientes' ? '🏖️' : '📝' }}
        </p>
        <p class="font-display font-bold text-xl text-text dark:text-dark-text mb-1">
