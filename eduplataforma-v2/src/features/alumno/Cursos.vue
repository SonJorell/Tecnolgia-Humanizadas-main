...
<template>
  <div class="p-4 md:p-6 space-y-6 animate-fade-in">
    <h1 class="font-display font-extrabold text-2xl text-text dark:text-dark-text">📚 Mis Cursos</h1>
    ...
    <div v-if="cursosStore.loading && cursosStore.inscripciones.length === 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      <div v-for="i in 4" :key="i" class="bg-surface dark:bg-dark-card2 h-32 rounded-xl animate-pulse" />
    </div>
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      ...
      >
        ...
        <div 
          class="absolute -top-10 -right-10 w-32 h-32 rounded-full opacity-[0.03] dark:opacity-[0.05] pointer-events-none"
          :style="{ backgroundColor: insc.curso?.color || '#1a6fa8' }"
        />
        
        <div class="flex items-start justify-between mb-2">
          <div>
            <p class="font-display font-bold text-base text-text dark:text-dark-text flex items-center gap-2">
              <span class="text-xl">{{ insc.curso?.icono }}</span>
              {{ insc.curso?.nombre }}
            </p>
            <p class="text-xs text-text-muted mt-0.5">{{ insc.curso?.nivel }}</p>
          </div>
        ...
        <div class="mt-4">
          ...
          <div class="h-2 bg-surface dark:bg-dark-card2 rounded-full overflow-hidden">
            <div
    ...
    <div v-if="!cursosStore.loading && cursosStore.inscripciones.length === 0" class="flex flex-col items-center justify-center py-12 text-center">
      <div class="w-16 h-16 bg-surface dark:bg-dark-card2 rounded-full flex items-center justify-center text-3xl mb-3">
        🎒
      </div>
      <h3 class="font-display font-bold text-lg text-text dark:text-dark-text">Aún no tienes cursos</h3>
