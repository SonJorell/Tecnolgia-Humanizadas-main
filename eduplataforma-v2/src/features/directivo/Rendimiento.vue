...
const distribucionNotas = ref([
  { rango: '1.0 - 2.9', cantidad: 12, color: 'bg-danger' },
  { rango: '3.0 - 3.9', cantidad: 28, color: 'bg-danger/60' },
  { rango: '4.0 - 4.9', cantidad: 45, color: 'bg-amber' },
  { rango: '5.0 - 5.9', cantidad: 85, color: 'bg-primary' },
  { rango: '6.0 - 6.9', cantidad: 110, color: 'bg-mint' },
  { rango: '7.0', cantidad: 35, color: 'bg-mint' },
])
...
function getColorForPromedio(promedio: number): string {
  if (promedio >= 6.0) return 'text-mint'
  if (promedio >= 5.0) return 'text-amber'
  return 'text-danger'
}

function getTendenciaColor(actual: number, anterior: number): string {
  if (actual > anterior) return 'text-mint'
  if (actual < anterior) return 'text-danger'
...
<template>
  <div class="p-4 md:p-6 max-w-7xl mx-auto space-y-6 animate-fade-in">
    ...
    <div>
      ...
      <p class="text-sm text-text-muted mt-1">Análisis profundo del rendimiento por cursos, asignaturas y docentes.</p>
    </div>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
      ...
      <StatCard
        ...
        iconBg="bg-danger-bg dark:bg-danger/20 text-danger"
      />
      <StatCard
        :icon="Users"
    ...
    <div class="grid grid-cols-1 lg:grid-cols-5 gap-6">
      ...
      <BaseCard class="lg:col-span-3">
        <h3 class="font-display font-bold text-base text-text dark:text-dark-text mb-5 flex items-center gap-2">
        ...
        <div class="space-y-3">
          ...
          >
            <span class="text-xs font-mono text-text-muted w-20 text-right flex-shrink-0">{{ d.rango }}</span>
            <div class="flex-1 h-7 bg-surface dark:bg-dark-card2 rounded-lg overflow-hidden relative">
              <div
                :class="['h-full rounded-lg transition-all duration-1000 ease-out', d.color]"
                :style="{ width: `${(d.cantidad / maxDistribucion) * 100}%` }"
              ></div>
              <span class="absolute right-2 top-1/2 -translate-y-1/2 text-[11px] font-bold text-text dark:text-dark-text">
                {{ d.cantidad }}
              </span>
            </div>
          </div>
      ...
      <div class="lg:col-span-2 space-y-4">
        <!-- Mejor curso -->
        <BaseCard class="border-l-4 border-mint">
          <div class="flex items-center gap-2 mb-3">
            <span class="text-xl">🏆</span>
            <h3 class="font-bold text-sm text-text dark:text-dark-text">Mejor Rendimiento</h3>
          </div>
          <p class="font-display font-extrabold text-lg text-mint">
          ...
          <div class="flex items-center gap-4 mt-2 text-xs text-text-muted">
            <span>Promedio <strong class="text-mint">{{ cursosDetalle.reduce((best, c) => c.promedio > best.promedio ? c : best).promedio.toFixed(1) }}</strong></span>
            <span>Entregas <strong class="text-mint">{{ cursosDetalle.reduce((best, c) => c.promedio > best.promedio ? c : best).tasa_entrega }}%</strong></span>
          </div>
        </BaseCard>

        <!-- Peor curso -->
        <BaseCard class="border-l-4 border-danger">
          <div class="flex items-center gap-2 mb-3">
            <span class="text-xl">⚠️</span>
            <h3 class="font-bold text-sm text-text dark:text-dark-text">Requiere Atención</h3>
          </div>
          <p class="font-display font-extrabold text-lg text-danger">
            {{ cursosDetalle.reduce((worst, c) => c.promedio < worst.promedio ? c : worst).nombre }}
          </p>
          <div class="flex items-center gap-4 mt-2 text-xs text-text-muted">
            <span>Promedio <strong class="text-danger">{{ cursosDetalle.reduce((worst, c) => c.promedio < worst.promedio ? c : worst).promedio.toFixed(1) }}</strong></span>
            <span>{{ cursosDetalle.reduce((worst, c) => c.promedio < worst.promedio ? c : worst).alumnos_riesgo }} en riesgo</span>
          </div>
        </BaseCard>

        <!-- Comparativa rápida -->
        <BaseCard class="bg-gradient-to-br from-violet-bg to-primary-bg dark:from-violet/10 dark:to-primary/10 border-0">
          <div class="text-center">
            <p class="text-xs font-semibold text-text-muted uppercase tracking-wider mb-1">Meta Institucional</p>
            <p class="font-display font-extrabold text-3xl text-primary">6.0</p>
    ...
    <!-- Filter Bar -->
    <div class="flex flex-col md:flex-row items-stretch md:items-center gap-3 bg-surface dark:bg-dark-card2 rounded-xl p-3">
      ...
      <div class="flex flex-wrap gap-2 flex-1">
        <select
          ...
          class="h-8 px-3 text-xs bg-card dark:bg-dark-card border border-border dark:border-white/10 rounded-lg outline-none focus:ring-2 focus:ring-primary/50 text-text dark:text-dark-text"
        >
      ...
      <div class="relative w-full md:w-56">
        ...
        <input
          ...
          class="w-full h-8 pl-8 pr-3 text-xs bg-card dark:bg-dark-card border border-border dark:border-white/10 rounded-lg outline-none focus:ring-2 focus:ring-primary/50 text-text dark:text-dark-text"
        />
      </div>
    </div>
    ...
    <BaseCard class="p-0 overflow-hidden">
        ...
        <table class="w-full text-left border-collapse min-w-[700px]">
          ...
          <tbody class="divide-y divide-border dark:divide-white/10 text-sm">
            ...
            >
              <td class="px-4 py-3">
                <div>
                  <p class="font-bold text-text dark:text-dark-text">{{ curso.nombre }}</p>
                  <p class="text-[11px] text-text-muted">{{ curso.asignatura }}</p>
                </div>
              </td>
              <td class="px-4 py-3 text-text-muted text-xs">{{ curso.profesor }}</td>
              ...
              <td class="px-4 py-3">
                <div class="flex items-center gap-2 justify-center">
                  <div class="w-20 h-2 bg-surface dark:bg-dark-card2 rounded-full overflow-hidden">
                    <div
                      class="h-full rounded-full transition-all duration-700"
                      :class="curso.tasa_entrega >= 80 ? 'bg-mint' : curso.tasa_entrega >= 50 ? 'bg-amber' : 'bg-danger'"
                      :style="{ width: `${curso.tasa_entrega}%` }"
                    ></div>
                  </div>
                  <span class="text-xs font-bold" :class="curso.tasa_entrega >= 80 ? 'text-mint' : curso.tasa_entrega >= 50 ? 'text-amber' : 'text-danger'">
                    {{ curso.tasa_entrega }}%
                  </span>
                </div>
              </td>
              ...
              <td class="px-4 py-3 text-center">
                <div class="text-xs">
                  <p class="font-medium text-text dark:text-dark-text">{{ curso.mejor_alumno }}</p>
                  <p class="text-mint font-bold">{{ curso.mejor_nota.toFixed(1) }}</p>
                </div>
              </td>
            </tr>
          </tbody>
      ...
      <div v-if="cursosFiltrados.length === 0" class="p-12 text-center">
        <Search :size="48" class="mx-auto text-text-light mb-3" />
        <p class="font-medium text-text dark:text-dark-text">Sin resultados</p>
        <p class="text-sm text-text-muted">Prueba cambiando los filtros de búsqueda.</p>
      </div>
    </BaseCard>
  </div>
</template>
