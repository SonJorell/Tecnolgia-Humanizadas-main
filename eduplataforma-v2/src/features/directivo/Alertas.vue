...
function getBgForTipo(tipo: string): string {
 switch (tipo) {
 case 'critica': return 'bg-error-container border-error/20'
 case 'advertencia': return 'bg-tertiary-container border-tertiary/20'
 default: return 'bg-primary-bg border-primary/20'
 }
}
...
<template>
 ...
 <p class="text-sm text-on-surface-variant mt-1">Monitoreo de incidencias críticas, advertencias y notificaciones del sistema.</p>
 </div>
 <BaseButton
 ...
 filtroTipo === 'critica'
 ? 'border-error bg-error-container shadow-md scale-[1.02]'
 : 'border-transparent bg-surface-container-lowest shadow-card hover:shadow-hover'
 ]"
 >
 <div class="flex items-center gap-2 mb-2">
 <AlertCircle :size="18" class="text-error" />
 <span class="text-xs font-bold text-error uppercase tracking-wider">Críticas</span>
 </div>
 <p class="font-headline-md font-extrabold text-3xl text-error">{{ countByType.criticas }}</p>
 </button>
 ...
 ? 'border-tertiary bg-tertiary-container shadow-md scale-[1.02]'
 : 'border-transparent bg-surface-container-lowest shadow-card hover:shadow-hover'
 ]"
 >
 <div class="flex items-center gap-2 mb-2">
 <AlertTriangle :size="18" class="text-tertiary" />
 <span class="text-xs font-bold text-tertiary uppercase tracking-wider">Advertencias</span>
 </div>
 <p class="font-headline-md font-extrabold text-3xl text-tertiary">{{ countByType.advertencias }}</p>
 </button>
 ...
 'rounded-xl p-4 text-left transition-all duration-200 border-2',
 filtroTipo === 'info'
 ? 'border-primary bg-primary-bg shadow-md scale-[1.02]'
 : 'border-transparent bg-surface-container-lowest shadow-card hover:shadow-hover'
 ]"
 >
 <div class="flex items-center gap-2 mb-2">
 ...
 <p class="font-headline-md font-extrabold text-3xl text-primary">{{ countByType.info }}</p>
 </button>

 <div class="bg-surface-container-lowest rounded-xl p-4 shadow-card">
 <div class="flex items-center gap-2 mb-2">
 <CheckCircle2 :size="18" class="text-secondary" />
 <span class="text-xs font-bold text-secondary uppercase tracking-wider">Resueltas</span>
 </div>
 <p class="font-headline-md font-extrabold text-3xl text-secondary">{{ countByType.resueltas }}</p>
 </div>
 </div>
 ...
 <!-- Status Tabs -->
 <div class="flex bg-surface-container-low rounded-xl p-1 flex-shrink-0">
 <button type="button"
 ...
 class="w-full h-9 pl-9 pr-3 text-xs bg-surface-container-lowest border border-border-subtle rounded-lg outline-none focus:ring-2 focus:ring-primary/50 text-on-surface"
 />
 </div>
 </div>
 ...
 alerta.resuelta ? 'bg-secondary-container ' : 
 alerta.tipo === 'critica' ? 'bg-error-container' :
 alerta.tipo === 'advertencia' ? 'bg-tertiary-container' : 'bg-primary/20'
 ]">
 <CheckCircle2 v-if="alerta.resuelta" :size="20" class="text-secondary" />
 ...
 alerta.resuelta ? 'text-on-surface-variant line-through' : 'text-on-surface '
 ]">
 {{ alerta.mensaje }}
 </p>
 ...
 class="w-8 h-8 rounded-lg bg-white/80 hover:bg-secondary-container flex items-center justify-center transition-colors group"
 title="Marcar como resuelta"
 >
 <CheckCircle2 :size="16" class="text-on-surface-variant group-hover:text-secondary transition-colors" />
 </button>
 <button type="button"
 v-else
 @click="reabrirAlerta(alerta.id)"
 class="w-8 h-8 rounded-lg bg-white/80 hover:bg-tertiary-container flex items-center justify-center transition-colors group"
 title="Reabrir alerta"
 >
 <Eye :size="16" class="text-on-surface-variant group-hover:text-tertiary transition-colors" />
 </button>
 </div>
 </div>
 </div>
 ...
 <!-- Empty State -->
 <div v-if="alertasFiltradas.length === 0" class="bg-surface-container-lowest rounded-2xl p-12 text-center shadow-card">
 <div class="w-16 h-16 mx-auto rounded-2xl bg-secondary-container flex items-center justify-center text-3xl mb-4">
 🎉
 </div>
 <h3 class="font-headline-md font-bold text-lg text-on-surface mb-2">
